            ...
            <div class="relative flex-1 px-4 py-6 w-[400px]" ref="tableContainer">
                ...
                <n-config-provider :theme-overrides="themeOverrides">
                    <n-data-table :columns="columns" :data="createData" bordered striped bottom-bordered
                        :row-key="rowKey"
                        :checked-row-keys="pageStore.execute_files.selects"
                ...
                <div class=" flex justify-end">
                    <n-button type="info" class=" mt-4" @click="pageStore.execute_files.show=false;">OK</n-button>
                </div>
            </div>
        </div>
    </div>
...
const createData = computed(()=>{
    const cs:DataRow[] = [];
    for (let i = 0; i < pageStore.execute_files.files.length; i++) {
        cs.push({
            id: pageStore.execute_files.files[i].id,
            filename: pageStore.execute_files.files[i].filename,
            description: pageStore.execute_files.files[i].description
        })
    }
    return cs;
})
...
function onCloseFiles(){
    pageStore.execute_files.show=false;
    // pageStore.execute_files.selects = [];
}

function handleCheck(rowKeys: DataTableRowKey[]) {
    pageStore.execute_files.selects = rowKeys as number[];
}
...
function onSaveDescription(){
    ...
    if(newDescription.value != currentFileInfo.description){
        pageStore.updateExecuteFileDescription(currentFileInfo.id, currentFileInfo.filename, newDescription.value);
